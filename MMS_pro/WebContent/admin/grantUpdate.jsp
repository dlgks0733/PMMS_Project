                        ...
                        <div class="form-group row">
                              ...
                              <div class="col-9">
                                 <select class="form-control" id="grant" name="grant">
                                 	<c:if test="${pVo.grant eq '0'}">
                                    <option value="">선택</option>
                                    <option value="0" selected="selected">일반사용자</option>
                                    <option value="1">PM</option>
                                    </c:if>
                                    <c:if test="${pVo.grant eq '1'}">
                                    <option value="">선택</option>
                        ...
                        <div class="kt-form__actions kt-form__actions--right">
                           <div class="row">
                              <div class="col kt-align-left">
                                 <button type="button" onclick="updateGrant()" class="btn btn-secondary">수정</button>
                              </div>
                           </div>
                        </div>
                     </div> 
...
function updateGrant(){
	...
	else if(confirm("권한 수정하시겠습니까?") == true) {
	$.ajax({
		
		type: 'POST',  // GET or POST 전송방법 
		url: '/prog?command=grantUpdate',  // 이쪽으로 보낸다(호출URL)
		...
		success: function(data){  // 만약 성공적으로 수행되었다면 result로 값반환
			alert("수정되었습니다.");
			self.close();
			opener.location.href = "/prog?command=grantListForm";
		},
		error: function(data){
			alert("오류:: 다시 시도해주세요.");
			return false;
